document.addEventListener("DOMContentLoaded", function () {
 ...
 function executeCommand(command) {
  ...
  switch (command) {
   case "Help":
    ...
    terminalOutput.innerHTML += "<li><strong>Exit</strong> - Close the terminal.</li>";
    terminalOutput.innerHTML += "</ul>";
   ...
   case "CV":
    ...
    case "Exit":
        terminalOutput.innerHTML += "<p>Closing Terminal...</p>";
        setTimeout(() => {
          window.location.href = "https://piyushpatilportfolio.netlify.app/"; 
        }, 1000);
    break;
 ...
 commandInput.addEventListener("keydown", function (event) {
  if (event.key === "Enter") {
   const command = commandInput.value.trim(); // Remove extra spaces
